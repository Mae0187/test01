...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        # [CORE] 初始化 QueueManager (取代舊的 CoreManager)
    ...
    def add_task_to_ui(self):
        ...
        # [KEY] 將 task_id 藏在 UserRole，以便後續查找
    ...
    def clear_finished_tasks(self):
        ...
        for row in sorted(rows_to_remove, reverse=True):
            # 也要記得通知後端移除 (雖然已完成，但保持狀態一致性)
            item = self.table.item(row, 0)
            if item:
                task_id = item.data(Qt.UserRole)
                self.queue_manager.cancel_task(task_id) # Cancel 會處理 cleanup
            self.table.removeRow(row)
    ...
    def update_task_progress(self, task_id, p_str, percent, speed, eta):
        ...
        if row is not None:
            ...
            if p_bar:
                ...
                # [FIX] 修改顯示格式： 下載速度 / 百分比
                # 範例: 2.5MiB/s / 45.5%
                p_bar.setFormat(f"{speed} / {p_str}") 
                
            # [Optional] 如果您想把 ETA 放到狀態欄，可以這樣寫：
            # self.table.item(row, 3).setText(f"剩餘: {eta}")
    ...
    def handle_task_error(self, task_id, error_msg):
        ...
        if row is not None:
            ...
            if p_bar:
                # 錯誤時進度條變紅或顯示錯誤
                p_bar.setFormat("錯誤: " + error_msg)
                # 這裡無法直接變色 chunk，除非重設 style，暫時顯示文字即可
