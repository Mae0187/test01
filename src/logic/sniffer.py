...
class BrowserSniffer:
    """
    瀏覽器自動嗅探器 (Phase 47: Range-Aware)
    修正: 優先檢查 Content-Range 總大小，解決串流影片因切片過小被誤刪的問題。
    """
    def __init__(self):
    ...
    def extract_stream_url(self, target_url: str) -> Tuple[Optional[str], Dict]:
        ...
        cwd = os.getcwd()
        profile_dir = os.path.join(cwd, "browser_profile_sniffer")
                ...
                if video_candidates:
                    for rid in reversed(video_candidates):
                        ...
                        if is_valid:
                            ...
                            self.logger.warning(f"❌ 目標未通過驗證 ({reason})，丟棄...")
    ...
    def _validate_media(self, driver, url: str, min_size: int, min_duration: int) -> Tuple[bool, str]:
        """
        [驗屍官 Phase 47] 優先檢查 Content-Range (總大小)
        """
        try:
            js_script = """
            var url = arguments[0];
            ...
            .then(response => {
                    ...
                    return response.text().then(text => {
                        ...
                        if (duration > minSecs) {
                            ...
                            if (text.includes('#EXTM3U')) {
                                 callback({valid: true, reason: 'M3U8 (No Duration info, but valid format)'});
                            } else {
                ...
                // 2. 檢查 MP4 / 串流檔
                    ...
                    // A. 優先解析 Content-Range: bytes 0-65535/52428800 (取分母)
                    if (cRange) {
                        var parts = cRange.split('/');
                        if (parts.length > 1 && parts[1] !== '*') {
                            totalSize = parseInt(parts[1]);
                        }
                    }
                    
                    // B. 如果沒有 Range，才看 Content-Length
                    if (totalSize === 0 && cLen) {
                        totalSize = parseInt(cLen);
                    }

                    if (totalSize > minBytes) {
                        var mb = (totalSize/1024/1024).toFixed(1);
                        callback({valid: true, reason: 'Size (Range-Aware): ' + mb + 'MB'});
                    } else if (totalSize > 0) {
                        var mb = (totalSize/1024/1024).toFixed(1);
    ...
    def _smart_bypass(self, driver) -> bool:
        ...
        try:
            xpaths = [
                "//*[contains(text(), '同意') or contains(text(), 'Yes') or contains(text(), 'Enter')]",
                "//*[contains(text(), '略過廣告') or contains(text(), '跳過廣告') or contains(text(), 'Skip Ad')]"
            ]
            for xp in xpaths:
            ...
            if not clicked:
                selectors = [".vjs-big-play-button", ".plyr__control--overlaid", ".art-control-play", "button[aria-label='Play']", ".html5-main-video"]
